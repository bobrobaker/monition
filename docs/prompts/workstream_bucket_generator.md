...
- 2026-07-02: Re-opening a gate-paused workstream is an in-place amendment, never a new workstream — add a revisit bucket, amend the gate invariant with a dated, user-ratified note (keep the original text; state exactly what is relaxed and what stands), and reconcile the verdict/decision docs in the same pass: the NO-GO doc gets an Update recording the re-open, and any decision extracted from the pause window (e.g. a stopgap shipped elsewhere while paused) gets a status note where its "workstream stays paused" claim went stale. One live answer everywhere. (From the relevance-cascade B06 re-open.)
